...
const baseUrl = () => {
  ...
  if (!url) throw new Error("GO_API_URL is not set");
  return url.replace(/\/$/, "");
};
...
export async function fetchFavorites(): Promise<CatDB[]> {
  ...
  if (!response.ok) {
    throw new Error("Error al obtener los datos de la API Go");
  }
...
export async function saveFavorite(payload: SaveCatPayload): Promise<CatDB> {
  ...
  if (!response.ok) {
    ...
    const message = (body as { detail?: string }).detail ?? "Error al guardar el gato en la API Go";
